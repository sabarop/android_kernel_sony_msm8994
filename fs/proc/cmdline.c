...
static int __init proc_cmdline_init(void)
{
	...
	a1 = strstr(updated_command_line, "androidboot.verifiedbootstate=");
	if (a1) {
            a1 = strchr(a1, '=');
            a2 = strchr(a1, ' ');
            ...
            scnprintf(proc_cmdline, COMMAND_LINE_SIZE, "%.*sgreen%s",
                          (int)(a1 - updated_command_line + 1),
                          updated_command_line, a2);
	} else {
            strncpy(proc_cmdline, updated_command_line, COMMAND_LINE_SIZE);
	}
